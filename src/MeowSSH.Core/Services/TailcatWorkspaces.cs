...
public sealed class FileTailcatWorkspaceStore(string path) : ITailcatWorkspaceStore
{
    ...
    public Task DeleteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        {
            var existing = ReadUnsafe();
            if (existing.RemoveAll(item => item.Id == id) > 0) WriteUnsafe(existing);
        }
        return Task.CompletedTask;
    }
    ...
    private List<TailcatWorkspace> ReadUnsafe()
    {
        ...
        {
            var json = File.ReadAllText(_path);
            return JsonSerializer.Deserialize(json, TailcatWorkspaceJsonContext.Default.TailcatWorkspaceDocument)?.Workspaces.Select(Normalize).ToList() ?? [];
        }
    ...
    private static TailcatWorkspace NormalizeAndValidate(TailcatWorkspace workspace)
    {
        ...
        if (normalized.Id == Guid.Empty) throw new ArgumentException("A workspace ID is required.", nameof(workspace));
        if (string.IsNullOrWhiteSpace(normalized.Name)) throw new ArgumentException("A workspace name is required.", nameof(workspace));
        if (normalized.Name.Length > 80) throw new ArgumentException("Workspace names must be 80 characters or fewer.", nameof(workspace));
        if (normalized.StartSocks && string.IsNullOrWhiteSpace(normalized.SocksListenAddress)) throw new ArgumentException("A SOCKS listen address is required when the workspace starts SOCKS.", nameof(workspace));
        if (normalized.Forwards.Count > 20) throw new ArgumentException("A workspace can contain at most 20 forwarding groups.", nameof(workspace));
        if (normalized.Forwards.Any(static forward => forward.Mappings.Count is < 1 or > 50)) throw new ArgumentException("Each forwarding group must contain between 1 and 50 mappings.", nameof(workspace));
        return normalized;
    }
    ...
    private static TailcatWorkspace Normalize(TailcatWorkspace workspace) => workspace with
    {
        ...
        SocksListenAddress = string.IsNullOrWhiteSpace(workspace.SocksListenAddress) ? "127.0.0.1:0" : workspace.SocksListenAddress.Trim(),
        Forwards = workspace.Forwards.Where(static forward => forward.Mappings.Count > 0).Select(static forward => forward with
        {
            Mappings = forward.Mappings.Where(static mapping => !string.IsNullOrWhiteSpace(mapping)).Select(static mapping => mapping.Trim()).Distinct(StringComparer.Ordinal).ToArray(),
            BindAddress = string.IsNullOrWhiteSpace(forward.BindAddress) ? "127.0.0.1" : forward.BindAddress.Trim(),
        }).Where(static forward => forward.Mappings.Count > 0).ToArray(),
    };
...
public sealed class MemoryTailcatWorkspaceStore : ITailcatWorkspaceStore
{
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        lock (_gate) return Task.FromResult<IReadOnlyList<TailcatWorkspace>>([.. _workspaces.Values.OrderBy(static item => item.Name, StringComparer.OrdinalIgnoreCase)]);
    }
    public Task SaveAsync(TailcatWorkspace workspace, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(workspace);
...
public sealed class TailcatWorkspaceService(ITailcatWorkspaceStore store, ITailcatHubService hub, IEntitlementService entitlements, TimeProvider? timeProvider = null) : ITailcatWorkspaceService
{
    private readonly object _stateGate = new();
    private readonly TimeProvider _timeProvider = timeProvider ?? TimeProvider.System;
    private TailcatWorkspaceActivation? _active;
    private bool _busy;
    public TailcatWorkspaceActivation? Active { get { lock (_stateGate) return _active; } }
    public event EventHandler? Changed;
    public Task<IReadOnlyList<TailcatWorkspace>> GetAllAsync(CancellationToken cancellationToken = default) => store.GetAllAsync(cancellationToken);
    ...
    {
        if (Active?.WorkspaceId == id) throw new InvalidOperationException("Stop the active workspace before deleting it.");
    ...
    public async Task StartAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            if (Active is not null) throw new InvalidOperationException("Stop the active Tailcat workspace before starting another one.");
            var workspace = (await store.GetAllAsync(cancellationToken).ConfigureAwait(false)).FirstOrDefault(item => item.Id == id) ?? throw new KeyNotFoundException("The Tailcat workspace no longer exists.");
            if (string.IsNullOrWhiteSpace(workspace.Address) && workspace.Forwards.Count > 0) throw new InvalidOperationException("A Tailcat server address is required when the workspace contains forwards.");
            ...
            try
            {
                ...
                {
                    await hub.StartSocksAsync(new TailcatSocksRequest(workspace.SocksListenAddress, workspace.ClientKey, workspace.DerpMapUrl), cancellationToken).ConfigureAwait(false);
                    ownsSocks = true;
                }
                foreach (var forward in workspace.Forwards)
                {
                    var snapshot = await hub.StartForwardAsync(new TailcatForwardRequest(workspace.Address, forward.Mappings, forward.BindAddress, workspace.ClientKey, workspace.DerpMapUrl, forward.Udp), cancellationToken).ConfigureAwait(false);
                    startedForwards.Add(snapshot.Id);
                }
                lock (_stateGate) _active = new TailcatWorkspaceActivation(workspace.Id, workspace.Name, ownsSocks, [.. startedForwards], _timeProvider.GetUtcNow());
                Changed?.Invoke(this, EventArgs.Empty);
            }
            catch
            {
                ...
                {
                    try { await hub.StopForwardAsync(startedForwards[index], CancellationToken.None).ConfigureAwait(false); } catch { }
                }
                if (ownsSocks)
                {
                    try { await hub.StopSocksAsync(CancellationToken.None).ConfigureAwait(false); } catch { }
                }
                throw;
            }
        }
    ...
    public async Task StopAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                try { await hub.StopForwardAsync(active.ForwardIds[index], cancellationToken).ConfigureAwait(false); } catch (Exception exception) { (failures ??= []).Add(exception); }
            }
            if (active.OwnsSocks)
            {
                try { await hub.StopSocksAsync(cancellationToken).ConfigureAwait(false); } catch (Exception exception) { (failures ??= []).Add(exception); }
            }
            lock (_stateGate) _active = null;
            Changed?.Invoke(this, EventArgs.Empty);
            if (failures is { Count: > 0 }) throw new AggregateException("One or more Tailcat workspace resources could not be stopped.", failures);
        }
        finally { ExitOperation(); }
    }
    ...
    {
        if (!entitlements.Has(PremiumFeature.TailcatWorkspaces)) throw new InvalidOperationException("Tailcat Workspaces require MeowSSH Pro.");
    }
    ...
    private void ExitOperation() { lock (_stateGate) _busy = false; }
}
